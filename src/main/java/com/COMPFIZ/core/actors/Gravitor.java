...
public class Gravitor {
    ...
    public void update(double interval, Entity planet, Entity[] masses) {
        Vector3d acc = new Vector3d();
        Vector3d force = new Vector3d();
        Vector3d[] gravitational = new Vector3d[masses.length];
        Physics stuff = (Physics) planet.physics;
        if(stuff.mass == 0) return;
        for (int i = 0; i < masses.length; i++){
            if(masses[i] == null || masses[i] == planet) continue;
    ...
    public Vector3d updated(double interval, Entity planet, Entity[] masses) {
        Vector3d acc = new Vector3d();
        Vector3d force = new Vector3d();
        Vector3d[] gravitational = new Vector3d[masses.length];
        Physics stuff = (Physics) planet.physics;
